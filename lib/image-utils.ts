...
export function getImageUrl(image: any, width: number = 1920): string {
  ...
  if (typeof image === 'string') {
    return image.startsWith('http') ? image : urlForImage(image).width(width).format('webp').quality(80).url();
  }

  // Handle Sanity image reference object
  if (image.asset) {
    return urlForImage(image).width(width).format('webp').quality(80).url();
  }
